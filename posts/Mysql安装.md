...
function mdContentHtml2AddSlideNav(mdContentHtml) {
    ...
    return $(mdContentHtml).append(articleSlideNavJq).prop('outerHTML');
}
...
function mdContentHtml2StyleCorrection(mdContentHtml) {
    ...
    //文章内的导航栏样式修正
    mdContentHtmlJq = mdContentHtmlJq
        .find('.table-of-contents:not(.article-slide-nav)')
    ...
    //图片样式修正
    mdContentHtmlJq = mdContentHtmlJq.find('img').css({ width: '80%' }).end();
    ...
    //删除mermaid语法内的复制按钮
    mdContentHtmlJq = mdContentHtmlJq.find('.mermaid').siblings('.markdown-it-code-copy').remove().end().end();

    //复制按钮添加标题
    mdContentHtmlJq = mdContentHtmlJq.find('.markdown-it-code-copy').text('复制').end();
...
function mdContentHtml2EventCorrection(mdContentHtml) {
    ...
    $(document).delegate('.article-slide-nav a', 'click', function () {
        $(this).parents('.article-slide-nav').find('a').css({
...
function mdToHtml2(mdContent) {
    ...
    if (strIsNotEmpty(mdContent)) {
        ...
        mdContentHtmlJquery = $("<div class='content-wrapper' />").append(mdContentHtml);

        //mermaid语法转换成svg在页面显示
        mdContentHtml = $(mdContentHtmlJquery)
            .find('.mermaid')
            .each(function (mermaidIndex) {
